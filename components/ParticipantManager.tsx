...
import { Trash2, Search, User, Check, Trophy, UserMinus, Sparkles } from 'lucide-react';
...
export const ParticipantManager: React.FC<ParticipantManagerProps> = ({ participants, setParticipants }) => {
  ...
  return (
    <div className="flex flex-col gap-8 pb-10 h-full pt-8 md:pt-12">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 shrink-0 px-4">
        <div className="text-center md:text-left">
          <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
            <Sparkles size={20} className="text-brand-primary" />
            <h2 className="text-4xl md:text-5xl font-black tracking-tighter text-white uppercase">全员名单</h2>
          </div>
          <p className="text-gray-500 font-bold tracking-[0.2em] uppercase text-[10px]">Staff Registry & Management</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1 min-h-0 items-start px-4">
        {/* Left Input Section */}
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="glass-card rounded-[32px] p-6 flex flex-col border border-white/5 shadow-2xl">
        ...
        <div className="lg:col-span-8 h-full overflow-y-auto pr-2 custom-scrollbar">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-10">
            {filtered.length > 0 ? filtered.map(p => (
              <div key={p.id} className={`glass-card p-5 rounded-3xl flex items-center justify-between group transition-all border ${p.isWinner ? 'border-brand-accent/20 bg-brand-accent/[0.02]' : 'border-white/5 shadow-lg hover:border-white/20'}`}>
              ...
              <div className="col-span-full py-16 text-center glass-card rounded-3xl opacity-30 border border-dashed border-white/10 flex flex-col items-center gap-3">
                <UserMinus size={32} />
